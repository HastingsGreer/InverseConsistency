...
d1_triangles, d2_triangles = data.get_dataset_triangles("train", data_size=50, hollow=True)
d1_triangles_test, d2_triangles_test = data.get_dataset_triangles("test",data_size=50, hollow=True)
...
print("=" * 50)
print(network, lmbda)
net = networks.InverseConsistentNet(network, lmbda, next(iter(d1))[0].size())
...
for _ in range(50):
    ...
    for N in range(3):
     visualize.visualizeRegistration(net, image_A, image_B, N, resultspath + "epoch" + str(_) + "case" + str(N) +  ".png")
random.seed(1)
torch.manual_seed(1)
torch.cuda.manual_seed(1)
np.random.seed(1)
