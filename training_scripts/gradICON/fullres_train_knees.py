...
def make_batch():
    image = torch.cat([random.choice(knees) for _ in range(GPUS * BATCH_SIZE)]).type(torch.float32) / 255.
    image = image.cuda()
    return image
...
for _ in range(0, 100000):
    ...
    loss_curve.append([torch.mean(l.detach().cpu()).item() for l in (a, b, c)] + [flips, fullres_net.lmbda])
    print(loss_curve[-1])
    optimizer.step()
    ...
    if _ % 300 == 0:
        ...
        torch.save(
            fullres_net.regis_net.state_dict(), footsteps.output_dir + "knee_aligner_resi_net" + str(_)
        )
